...
public class SaleDaoAndroid implements SaleDao {
	...
	@Override
	public void endSale(Sale sale, String endTime,String status) {
		ContentValues content = new ContentValues();
	...
	public List<Sale> getAll_local_Sale() {
		return getAllSaleWithItems(" WHERE status = 'wiat_syncserver'");
	}
	 ...
	 * @return
	 */
	public List<Sale> getAllSaleWithItems(String condition) {
		String queryString = "SELECT * FROM " + DatabaseContents.TABLE_SALE + condition;
        List<Object> objectList = database.select(queryString);
        List<Sale> list = new ArrayList<Sale>();
        for (Object object: objectList) {
